...
- ✅ Mirrors audio from current *default* render device to the first device whose name contains **`TV`** (case-insensitive)
...
- Windows 10/11
- .NET 6/7/8 Desktop Runtime (if you publish *framework-dependent*)
...
```bash
dotnet restore
dotnet publish -c Release -r win-x64 -p:PublishSingleFile=true --self-contained false -o ./publish/win-x64
